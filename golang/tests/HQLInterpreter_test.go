...
func TestSchemaTable(t *testing.T) {
	r := &registry.OrmRegistry{}
	r.Register(&tests.Node{}, nil)
	st, _ := r.Schema().SchemaNode("node")
	si := st.NewStructInstance(nil)
	newInstances := si.ValueOfOrCreate(reflect.ValueOf(nil))
	node := newInstances[0].Value().Interface().(*tests.Node)
	if node == nil {
		t.Fail()
		Error("Node is nil")
		return
	...
	st, _ = r.Schema().SchemaNode("node.Ptr")
	si = st.NewStructInstance(nil)
	newInstances = si.ValueOfOrCreate(reflect.ValueOf(nil))
	addr := newInstances[0].Value().Interface().(*tests.Node)
	if addr == nil {
		t.Fail()
	...
	sf := r.Schema().CreateAttributeID("node.Ptr.string")
...
func TestMapSetValueTableCreation(t *testing.T) {
	...
	r := &registry.OrmRegistry{}
	r.Register(&tests.Node{}, nil)
	schemaNode, keys := r.Schema().SchemaNode(path)
	if schemaNode == nil {
		t.Fail()
		Error("Could not find any schema node:" + path)
		return
	}
	si := schemaNode.NewStructInstance(keys)
	newInstances := si.ValueOfOrCreate(reflect.ValueOf(nil))
	node6 := newInstances[0].Value().Interface().(*tests.SubNode6)
	if node6 == nil {
		t.Fail()
...
func TestMapSetValue(t *testing.T) {
	r := &registry.OrmRegistry{}
	r.Register(&tests.Node{}, nil)
	schemaField := r.Schema().CreateAttributeID("node.MapPrimary[k1].String")
	...
	schemaField = r.Schema().CreateAttributeID("node.MapPrimary[k2].String")
	schemaField.SetValue(node, expected)
